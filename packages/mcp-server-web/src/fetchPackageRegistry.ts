import { PackageRegistry, PackageResolveContext, PackageSpec } from "@myriaddreamin/typst.ts/internal.types";
import { WritableAccessModel } from "@myriaddreamin/typst.ts/fs/index";
...
export class FetchPackageRegistry implements PackageRegistry {
  ...
  resolve(spec: PackageSpec, context: PackageResolveContext): string | undefined {
    ...
    /// Fetch data
    const data = await this.pullPackageData(spec);
    if (!data) {
      return undefined;
    }
